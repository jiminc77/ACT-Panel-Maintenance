...
def main(args):
    ...
    if not is_eval:
        wandb.init(project="mobile-aloha2", reinit=True, entity="lghjayly", name=expr_name)
        wandb.config.update(config)
    with open(config_path, 'wb') as f:
        pickle.dump(config, f)
    if is_eval:
        # Use specific checkpoint if provided, otherwise use policy_last.ckpt
        if 'eval_ckpt_name' in args and args['eval_ckpt_name'] is not None:
            ckpt_names = [args['eval_ckpt_name']]
        else:
        ...
        for ckpt_name in ckpt_names:
            try:
                ...
                # Fallback: dataset-based eval when sim env is not implemented for this task
                print('Sim env not implemented for this task. Falling back to dataset-based eval (val loss).')
                # build dataloaders for val set
                train_dataloader, val_dataloader, _, _ = load_data(
                    dataset_dir, name_filter, camera_names,
                    batch_size_train, batch_size_val,
                    args['chunk_size'], args['skip_mirrored_data'],
                    config['load_pretrain'], policy_class,
                    stats_dir_l=stats_dir, sample_weights=sample_weights,
                    train_ratio=train_ratio
                )
                # load policy and checkpoint
                policy = make_policy(policy_class, policy_config)
                ckpt_path = os.path.join(ckpt_dir, ckpt_name)
                _ = policy.deserialize(torch.load(ckpt_path))
                policy.cuda()
                ds_val_loss = eval_on_dataset(policy, val_dataloader)
                print(f'eval_val_loss: {ds_val_loss}')
                results.append([ckpt_name, None, ds_val_loss])

        for ckpt_name, success_rate, avg_return in results:
            if success_rate is None:
                print(f'{ckpt_name}: eval_val_loss={avg_return}')
            else:
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    #     actuator_unnor`   m = lambda x: x * actuator_stats['commanded_speed_std'] + actuator_stats['commanded_speed_std']
...
def eval_on_dataset(policy, val_dataloader):
    policy.eval()
    losses = []
    with torch.inference_mode():
        for batch_idx, data in enumerate(val_dataloader):
            forward_dict = forward_pass(data, policy)
            losses.append(forward_dict['loss'].detach().cpu())
            if batch_idx > 50:
                break
    if len(losses) == 0:
        return None
    ...
    for step in tqdm(range(num_steps+1)):
        ...
        if (step > 0) and (eval_every > 0) and (step % eval_every == 0):
            ...
            try:
                ...
                # Fallback: dataset-based eval
                print('Sim env not implemented for this task. Falling back to dataset-based eval (val loss).')
                ds_val_loss = eval_on_dataset(policy, val_dataloader)
                if ds_val_loss is not None:
                    wandb.log({'eval_val_loss': ds_val_loss}, step=step)
                else:
                    print('Validation dataloader empty; skipping eval.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_ckpt_name', action='store', type=str, help='specific checkpoint name for evaluation', required=False)
