...
class RobotEnv:
    def __init__(
        ...
        camera_read_interval: int = 1,
    ) -> None:
        self._robot = robot
        self._rate = Rate(control_rate_hz)
        self._camera_dict = {} if camera_dict is None else camera_dict
        self._camera_read_interval = camera_read_interval
        self._camera_frame_count = 0
        self._last_camera_images = {}  # 마지막 카메라 이미지 저장
    ...
    def get_obs(self) -> Dict[str, Any]:
        ...
        # 카메라 읽기 주기 조절
        should_read_camera = (self._camera_read_interval == 1 or 
                             self._camera_frame_count % self._camera_read_interval == 0)
        
        for name, camera in self._camera_dict.items():
            if should_read_camera:
                # 실제로 카메라에서 읽기
                image, depth = camera.read()
                self._last_camera_images[name] = (image, depth)
            else:
                # 이전 프레임 재사용
                if name in self._last_camera_images:
                    image, depth = self._last_camera_images[name]
                else:
                    # 첫 프레임이면 읽기
                    image, depth = camera.read()
                    self._last_camera_images[name] = (image, depth)
